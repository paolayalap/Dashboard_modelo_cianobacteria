...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        ...
        if (("oxigeno" in k or "oxygen" in k or "dissolved oxygen" in k or re.search(r"\bdo\b", k) or re.search(r"\bod\b", k) or "o2" in k)
            and ("mg/l" in k or "mg l" in k or "mg" in k)): ren[c] = "Oxígeno Disuelto (mg/L)"; continue
        if "clorofila" in k or "chlorophyll" in k: ren[c] = TARGET_CHL; continue
        ren[c] = c
...
def align_proba_to_labels(proba: np.ndarray, classes_pred, labels_order):
    ...
    np.divide(out, row_sums, out=out, where=row_sums > 0)  # asegura suma=1 en columnas mapeadas
    return out
...
# Autodetección de OD si viene con otro nombre
if "Oxígeno Disuelto (mg/L)" not in df_cea.columns:
    cands = []
    for col in df_cea.columns:
        kc = _canon(col)
        if (("oxigeno" in kc or "oxygen" in kc or "dissolved oxygen" in kc or re.search(r"\bdo\b", kc) or re.search(r"\bod\b", kc) or "o2" in kc)
...
st.subheader("📈 Curvas de entrenamiento (regresión continua — Clorofila)")
TRAIN_SCALER = None
...
if not KERAS_OK:
    ...
    ax.plot(losses, label="Pérdida entrenamiento"); ax.plot(val_losses, label="Pérdida validación")
    ...
    if not base_chl.empty:
        ...
        X_tr, X_te, y_tr, y_te = train_test_split(Xc, yc, test_size=0.20, random_state=42)
        ...
        st.info("No hay suficientes filas válidas para entrenar el regresor de Clorofila.")
...
if not base_cls_chl.empty:
    ...
    X_train, y_train_cls = X_train[mask_ok], y_train_cls[mask_ok]
...
# ------------------------- 4) Estanque: Predicción + MATRICES DIFUSAS (verdad = NN) -------------------------
st.subheader("🧪 Estanque — Predicción (NN) y Matrices **Difusas**")
...
if clicked:
    ...
    # --- Predicción continua con NN (VERDAD PARA LA MATRIZ) ---
    if (TRAIN_SCALER is not None) and (TRAIN_MODEL_CHL is not None):
        ...
        st.error("El regresor de NN no está disponible. Debes entrenarlo en la sección superior.")
        st.stop()
    yhat_chl = np.clip(yhat_chl, 0.0, None)

    # --- Probabilidades por clase (para el eje predicho de la matriz) ---
    ...
    # --- "Verdad" difusa del estanque = PREDICCIÓN CONTINUA NN ---
    y_true_chl = yhat_chl.copy()  # TODAS las filas aportan 1 → suma de pesos = Npond
    ...
    if proba_svm_p_al is not None:
        ...
        with cA:
            ...
            st.caption(f"Suma de pesos (SVM): {cm_svm_p.sum():.2f}  |  Filas usadas: {Npond}")
    if proba_knn_p_al is not None:
        ...
        with cB:
            ...
            st.caption(f"Suma de pesos (KNN): {cm_knn_p.sum():.2f}  |  Filas usadas: {Npond}")

    st.info("ℹ️ En el estanque, la **etiqueta real (difusa)** proviene de la **predicción continua del modelo NN**. Por eso la **suma de pesos = número de filas**.")
    ...
    st.success("✅ Estanque: predicciones (NN) y matrices difusas generadas.")
